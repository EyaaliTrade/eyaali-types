...
pub struct GetPromoCodeDetailsBody {
    pub code: String
}

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct GetPromoCodeDetailsResult {
    pub promo_code: Option<PromoCodeAggregation>,
}
...
pub struct PromoCodeTargetsAggregation {
    pub kind: Option<String>,
    pub customers: Vec<PromoCodeCustomerIdAggregation>,
    pub groups: Vec<PromoCodeGroupIdAggregation>,
    pub subscribers: Vec<PromoCodeSubscriberIdAggregation>,
}
...
pub enum GetPromoCodeDetailsError {
    #[display(fmt = "invalid_object_id")]
    InvalidObjectId,
    Default(String),
}

impl ResponseError for GetPromoCodeDetailsError {
    fn error_response(&self) -> HttpResponse {
        match self {
            GetPromoCodeDetailsError::InvalidObjectId => {
                HttpResponse::NotAcceptable().body("invalid_object_id")
            }
            GetPromoCodeDetailsError::Default(error) => HttpResponse::BadRequest().body(error),
        }
    }
}